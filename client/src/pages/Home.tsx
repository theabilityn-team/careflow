    ...
    <div className="mx-auto max-w-[1500px]">
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.55fr_.85fr]">
        ...
        <Card className="rounded-2xl border-0 bg-slate-950 text-white shadow-[0_12px_40px_rgba(15,23,42,.14)]">
          ...
          <CardContent className="px-6 pb-6">
            {!followUps?.length ? <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-5 text-sm leading-6 text-slate-400">No follow-ups are scheduled. Add the next action while logging a contact.</div> : <div className="space-y-3">{followUps.slice(0, 5).map(lead => <button key={lead.id} onClick={() => navigate(`/leads/${lead.id}`)} className="flex w-full items-center justify-between gap-4 rounded-xl bg-white/[.06] p-4 text-left transition-colors hover:bg-white/[.1]"><div className="min-w-0"><p className="truncate text-sm font-medium">{lead.firstName} {lead.lastName}</p><p className="mt-1 text-xs text-slate-400">{formatDate(lead.nextFollowUpAt, true)}</p></div><ArrowRight className="h-4 w-4 shrink-0 text-teal-300" /></button>)}</div>}
          </CardContent>
        </Card>
      </div>
    </div>
