...
export default function Home() {
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      <PageHeader eyebrow="Operations overview" title="Good work starts with a clear queue." description="Monitor lead momentum, review recent records, and keep every follow-up on schedule." actions={access?.permissions.scanDocuments && access.permissions.viewClinical ? <Button onClick={() => navigate("/scan")} className="w-full bg-teal-700 hover:bg-teal-800 sm:w-auto"><ScanLine className="mr-2 h-4 w-4" />Add lead from images</Button> : undefined} />
      {error ? <div className="mb-6 break-words rounded-xl bg-amber-50 p-4 text-sm text-amber-900 ring-1 ring-amber-200 [overflow-wrap:anywhere]">{error.message}</div> : null}

      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {stats.map(stat => <Card key={stat.label} className="rounded-2xl border-0 bg-white shadow-[0_8px_30px_rgba(15,23,42,.045)]"><CardContent className="p-5"><div className="flex min-w-0 items-start justify-between gap-3"><div className="min-w-0"><p className="break-words text-sm font-medium text-slate-500">{stat.label}</p><p className="mt-3 break-words text-3xl font-semibold tracking-[-.04em] text-slate-950">{stat.value}</p></div><div className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${stat.tone}`}><stat.icon className="h-5 w-5" /></div></div></CardContent></Card>)}
      </div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.55fr_.85fr]">
        <Card className="rounded-2xl border-0 bg-white shadow-[0_8px_30px_rgba(15,23,42,.045)]">
          <CardHeader className="flex flex-col items-start justify-between gap-2 space-y-0 p-5 sm:flex-row sm:items-center sm:p-6"><div className="min-w-0"><CardTitle className="text-lg tracking-tight">Recently updated</CardTitle><p className="mt-1 break-words text-sm text-slate-500">Latest activity across your pipeline</p></div><Button variant="ghost" size="sm" onClick={() => navigate("/leads")} className="self-start sm:self-auto">View all <ArrowRight className="ml-2 h-4 w-4" /></Button></CardHeader>
          <CardContent className="px-5 pb-5 sm:px-6 sm:pb-6">
            {!data?.recent.length ? <EmptyState title="No leads yet" description="Upload images for one person, review the extracted details, and create the first lead." action={<Button onClick={() => navigate("/scan")} variant="outline"><UserRoundPlus className="mr-2 h-4 w-4" />Add first lead</Button>} /> : <div className="divide-y divide-slate-100">{data.recent.map(lead => <button key={lead.id} onClick={() => navigate(`/leads/${lead.id}`)} className="flex min-h-16 w-full min-w-0 items-start gap-3 py-4 text-left transition-colors hover:bg-slate-50/70 sm:items-center sm:gap-4"><Avatar className="h-10 w-10 shrink-0"><AvatarFallback className="bg-slate-100 text-xs font-semibold text-slate-700">{initials(lead.firstName, lead.lastName)}</AvatarFallback></Avatar><div className="min-w-0 flex-1"><p className="break-words text-sm font-semibold text-slate-900 [overflow-wrap:anywhere]">{lead.firstName} {lead.lastName}</p><p className="mt-1 break-all text-xs text-slate-500">{lead.email || lead.phone || "No contact information"}</p><div className="mt-2 sm:hidden"><StatusPill value={lead.status} /></div></div><div className="hidden shrink-0 sm:block"><StatusPill value={lead.status} /></div><span className="hidden w-24 shrink-0 text-right text-xs text-slate-400 sm:block">{formatDate(lead.updatedAt)}</span></button>)}</div>}
          </CardContent>
        </Card>
