...
import { ArrowRight, BellRing, CheckCircle2, ClipboardCheck, Download, FileHeart, Files, FileSearch, FolderKanban, History, KeyRound, MessageSquarePlus, ScanLine, ServerCog, ShieldCheck, UserCog } from "lucide-react";
...
const flows = [
  ...
  },
  {
    ...
    steps: ["Set a Scheduled follow-up from Log contact or Edit lead.", "Select English or Spanish on the lead record; this is the lead's reminder language, not a staff preference.", "Use Queue for overdue and upcoming work, or Calendar to select a day and review its schedule.", "Two hours before the appointment, an unread in-app notification appears. CareFlow uses the assigned staff member's verified SMTP account: the staff email is always English and the lead email uses that lead's selected language.", "After the call, email, or meeting, select Complete and record the method, outcome, notes, and optional next reminder.", "Use Archive to search and filter completed follow-ups while preserving the original scheduled time, completion time, staff actor, result, and reschedule decision."],
    result: "Active reminders remain actionable, emails come from the responsible staff member, and every completed follow-up becomes a permanent, permission-scoped history record without changing Business Status or Interest Signal.",
  },
  {
  ...
  {
    number: "11", icon: ServerCog, title: "Connect a staff SMTP mailbox", action: "Open Email settings", path: "/email-settings",
    steps: ["Each technical staff member opens Email settings.", "Enter the SMTP host, port, TLS mode, username, password, sender email, and sender name supplied by that mailbox provider.", "Enable automatic sending and save the settings.", "Select Test connection, then Send test email.", "Super Admin monitors readiness under Staff & access without seeing SMTP passwords.", "If required, an administrator may update the same values directly in the staff_smtp_settings database table."],
    result: "Follow-up emails for leads assigned to that staff member are sent from that staff member's mailbox. A saved SMTP password is never returned by the API or shown in the UI.",
  },
];
