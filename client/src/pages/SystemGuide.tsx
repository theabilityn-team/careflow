...
const flows = [
  ...
  },
  {
    ...
    steps: ["Open a lead and select Send email beside Edit lead and Contact & follow-up, or use the central Mails workspace.", "Optionally select an active product and an approved Super Admin plain-text or HTML template.", "Review and personalize the filled subject and message. HTML templates can be previewed or edited as source before sending.", "Super Admin can open Mails → Header & footer to edit or copy both HTML sections and switch to an exact sanitized live preview with sample content before saving.", "CareFlow sanitizes HTML again, replaces optional lead and sender tokens, wraps every message with the global HTML header and footer, adds a private open pixel, and converts safe web links to signed CareFlow click redirects.", "Send through the logged-in user's active, verified SMTP account.", "Open the lead's Emails tab to review the exact successful and failed history across every staff sender, plus first/latest open and click counts in Eastern Time; successful sends also appear in Communication history.", "Treat engagement as an estimate: image blocking may hide opens, while privacy protection and security scanners may create automatic opens or clicks. CareFlow stores no recipient IP address, device, or browser fingerprint.", "To read replies or any received email, sign in directly to the assigned email mailbox."],
    result: "Staff can send tracked outbound email from an individual lead or the Mails workspace without seeing SMTP credentials. Each lead shows cross-staff delivery and approximate open/click engagement with sender, product, template, recipient, status, time, and message preview. CareFlow does not act as an inbox.",
  },
  {
...
export default function SystemGuide() {
  ...
  return <div className="mx-auto max-w-[1380px]">
    ...
    <div className="space-y-5">{flows.map(flow => <Card key={flow.number} className="min-w-0 rounded-2xl border-0 bg-white shadow-[0_8px_30px_rgba(15,23,42,.04)]"><CardContent className="min-w-0 p-4 sm:p-7"><div className="flex min-w-0 flex-col gap-5 lg:flex-row lg:items-start"><div className="flex min-w-0 items-start gap-3 sm:gap-4 lg:min-w-72"><div className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-teal-50 text-teal-700"><flow.icon className="h-5 w-5" /></div><div className="min-w-0"><p className="text-xs font-semibold tracking-[.16em] text-teal-700">FLOW {flow.number}</p><h2 className="mt-1 break-words text-lg font-semibold text-slate-950 [overflow-wrap:anywhere]">{flow.title}</h2><Button variant="outline" size="sm" className="mt-4 h-auto min-h-11 max-w-full whitespace-normal text-left leading-5 md:min-h-8" onClick={() => navigate(flow.path)}><span className="break-words">{flow.action}</span><ArrowRight className="ml-2 h-3.5 w-3.5 shrink-0" /></Button></div></div><div className="grid min-w-0 flex-1 gap-5 md:grid-cols-[1fr_.8fr]"><ol className="min-w-0 space-y-3">{flow.steps.map((step, index) => <li key={step} className="flex min-w-0 gap-3 break-words text-sm leading-6 text-slate-600 [overflow-wrap:anywhere]"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-slate-100 text-[11px] font-semibold text-slate-600">{index + 1}</span><span className="min-w-0">{step}</span></li>)}</ol><div className="min-w-0 rounded-xl bg-emerald-50 p-4"><p className="text-xs font-semibold uppercase tracking-[.14em] text-emerald-700">Result</p><p className="mt-2 break-words text-sm leading-6 text-emerald-900 [overflow-wrap:anywhere]">{flow.result}</p></div></div></div></CardContent></Card>)}</div>

    <Card className="mt-8 min-w-0 rounded-2xl border-0 bg-white shadow-sm"><CardHeader><CardTitle>Lead status dictionary</CardTitle><p className="break-words text-sm leading-6 text-slate-500">Status answers one question: <strong>What business stage is this lead in now?</strong> It does not represent the follow-up reminder date.</p></CardHeader><CardContent><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{STATUS_OPTIONS.map(([value]) => <div key={value} className="min-w-0 rounded-xl border border-slate-100 p-4"><div className="flex min-w-0 flex-col items-start gap-2 min-[360px]:flex-row min-[360px]:items-center min-[360px]:justify-between"><StatusPill value={value} /><Badge variant="outline" className="max-w-full whitespace-normal break-all font-mono text-[10px] text-slate-400">{value}</Badge></div><p className="mt-3 break-words text-sm leading-6 text-slate-600 [overflow-wrap:anywhere]">{STATUS_DESCRIPTIONS[value]}</p></div>)}</div></CardContent></Card>
