...
import { LeadEmailComposerDialog, LeadEmailHistory } from "@/components/crm/LeadEmail";
...
import { parseEasternDateTimeInput } from "@shared/time";
...
export default function LeadDetail() {
  ...
  return <div className="mx-auto max-w-[1380px]">
    <button onClick={() => navigate("/leads")} className="mb-5 flex min-h-11 items-center gap-2 rounded-lg text-sm font-medium text-slate-500 hover:text-slate-900 md:min-h-8"><ArrowLeft className="h-4 w-4" />Back to leads</button>
    <PageHeader eyebrow={`Lead #${lead.id}`} title={`${lead.firstName} ${lead.lastName}`} description={`Created ${formatDate(lead.createdAt)} · Last updated ${formatDate(lead.updatedAt, true)}`} actions={<div className="grid w-full min-w-0 grid-cols-1 gap-2 min-[360px]:grid-cols-2 sm:flex sm:w-auto sm:flex-wrap sm:justify-end">
      {access?.permissions.editLeads && <div className="min-w-0 [&>button]:w-full sm:[&>button]:w-auto"><EditLeadDialog lead={lead} assignees={assignees} canViewClinical={access.permissions.viewClinical} canChangeStatus={access.permissions.changeStatus} onSave={async values => { await update.mutateAsync({ id, lead: values as any }); toast.success("Profile details saved and audited."); }} /></div>}
      {access?.permissions.manageContacts && <div className="min-w-0 [&>button]:w-full sm:[&>button]:w-auto"><ContactDialog onSave={async values => { await addContact.mutateAsync({ leadId: id, ...values }); toast.success("Contact activity and follow-up saved."); }} /></div>}
      {access?.permissions.manageContacts && <div className="min-w-0 min-[360px]:col-span-2 [&>button]:w-full sm:col-span-1 sm:[&>button]:w-auto"><LeadEmailComposerDialog lead={lead} onSent={async () => { await Promise.all([emailHistory.refetch(), utils.leads.get.invalidate({ id }), utils.mail.history.invalidate(), utils.dashboard.invalidate()]); }} /></div>}
    </div>} />
    ...
    <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
      ...
      <Card className="min-w-0 rounded-2xl border-0 bg-white shadow-sm"><CardContent className="min-w-0 p-5"><p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Lead groups</p><div className="mt-3 flex min-h-10 min-w-0 flex-wrap items-center gap-1.5">{leadSharing?.groups.length ? leadSharing.groups.map(group => <Badge key={group.id} variant="outline" className="max-w-full whitespace-normal break-words bg-slate-50 text-left text-slate-700 [overflow-wrap:anywhere]">{group.name}</Badge>) : <span className="text-sm font-medium text-slate-500">No group</span>}</div><p className="mt-2 text-xs leading-5 text-slate-500">New leads use the creator's latest group by default.</p></CardContent></Card>
      <Card className="rounded-2xl border-0 bg-white shadow-sm"><CardContent className="p-5"><p className="text-xs font-semibold uppercase tracking-wider text-slate-400">Most recent contact</p><p className="mt-3 text-sm font-semibold text-slate-900">{lead.lastContactAt ? formatDate(lead.lastContactAt, true) : "No contact logged"}</p><p className="mt-2 text-xs leading-5 text-slate-500">Updated automatically when Contact & follow-up is saved.</p></CardContent></Card>
      <Card className="rounded-2xl border-0 bg-slate-950 text-white shadow-sm"><CardContent className="p-5"><p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Next follow-up</p><p className="mt-3 text-sm font-semibold">{lead.nextFollowUpAt ? formatDate(lead.nextFollowUpAt, true) : "No reminder scheduled"}</p><p className={`mt-2 text-xs leading-5 ${followUpTiming === "overdue" ? "text-amber-300" : "text-slate-400"}`}>{data.followUps.length} active reminder{data.followUps.length === 1 ? "" : "s"} · {followUpTimingLabel(followUpTiming)}.</p></CardContent></Card>
    </div>
    ...
    <Tabs defaultValue="overview" className="space-y-5"><TabsList className="h-auto min-h-11 flex-wrap justify-start rounded-xl bg-white p-1 shadow-sm"><TabsTrigger value="overview" className="rounded-lg">Overview</TabsTrigger><TabsTrigger value="documents" className="rounded-lg">Documents ({data.documents.length})</TabsTrigger><TabsTrigger value="communications" className="rounded-lg">Communications ({data.communications.length})</TabsTrigger><TabsTrigger value="emails" className="rounded-lg">Emails ({emailHistory.data?.sentCount ?? 0})</TabsTrigger><TabsTrigger value="audit" className="rounded-lg">Audit trail ({data.auditEvents.length})</TabsTrigger></TabsList>
      ...
      <TabsContent value="emails" className="mt-0"><LeadEmailHistory leadId={id} history={emailHistory.data} isLoading={emailHistory.isLoading} /></TabsContent>
...
function ContactDialog({ onSave }: { onSave: (values: any) => Promise<void> }) {
  ...
  async function save() { if (!outcome.trim()) return toast.error("Add an outcome."); const nextFollowUpAt = followUp ? parseEasternDateTimeInput(followUp) : undefined; if (followUp && !Number.isFinite(nextFollowUpAt)) return toast.error("Select a valid Eastern Time date and time."); setSaving(true); try { await onSave({ method, direction, outcome, notes: notes || null, contactedAt: Date.now(), nextFollowUpAt }); setOpen(false); setOutcome(""); setNotes(""); setFollowUp(""); } finally { setSaving(false); } }
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button className="bg-teal-700 hover:bg-teal-800"><MessageSquarePlus className="mr-2 h-4 w-4" />Contact & follow-up</Button></DialogTrigger><DialogContent><DialogHeader><DialogTitle>Record contact and follow-up</DialogTitle><DialogDescription>{CONTACT_TRACKING_HELP.contactFollowUp}</DialogDescription></DialogHeader><div className="grid gap-4 py-2 sm:grid-cols-2"><div className="space-y-2"><Label>Method</Label><Select value={method} onValueChange={setMethod}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["phone", "email", "sms", "in_person", "other"].map(v => <SelectItem key={v} value={v}>{v.replace("_", " ")}</SelectItem>)}</SelectContent></Select></div><div className="space-y-2"><Label>Direction</Label><Select value={direction} onValueChange={setDirection}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="outbound">Outbound</SelectItem><SelectItem value="inbound">Inbound</SelectItem></SelectContent></Select></div><div className="space-y-2 sm:col-span-2"><Label>Contact outcome *</Label><Input value={outcome} onChange={e => setOutcome(e.target.value)} placeholder="Interested — requested product details" /></div><div className="space-y-2 sm:col-span-2"><Label>Notes</Label><Textarea value={notes} onChange={e => setNotes(e.target.value)} rows={4} /></div><div className="space-y-2 sm:col-span-2"><Label>Schedule another follow-up (optional, ET)</Label><Input type="datetime-local" value={followUp} onChange={e => setFollowUp(e.target.value)} /><p className="text-xs leading-5 text-slate-500">A date adds a new independent reminder. Existing follow-ups remain unchanged and can be edited or deleted in Follow-ups.</p></div></div><DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button onClick={save} disabled={saving}>Save contact & follow-up</Button></DialogFooter></DialogContent></Dialog>;
}
...
function LeadAccessDialog({ leadId, staff }: { leadId: number; staff: Array<{ id: number; name: string | null; email: string | null }> }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="outline" className="border-slate-700 bg-slate-900 text-white hover:bg-slate-800 hover:text-white"><Share2 className="mr-2 h-4 w-4" />Groups & sharing</Button></DialogTrigger><DialogContent className="sm:max-w-xl"><DialogHeader><DialogTitle>Groups and staff access</DialogTitle><DialogDescription>Add this lead to one of your groups or explicitly share it with another staff member. Shared staff can open and work the lead.</DialogDescription></DialogHeader><div className="min-w-0 space-y-6 py-2">{availableGroups.length > 0 && <section className="min-w-0"><Label>Add to a group you manage</Label><div className="mt-2 flex min-w-0 flex-col gap-2 sm:flex-row"><Select value={groupId} onValueChange={setGroupId}><SelectTrigger className="w-full min-w-0"><SelectValue placeholder="Select group" /></SelectTrigger><SelectContent>{availableGroups.map(group => <SelectItem key={group.id} value={String(group.id)}>{group.name}</SelectItem>)}</SelectContent></Select><Button className="w-full sm:w-auto" disabled={!groupId || addToGroup.isPending} onClick={async () => { await addToGroup.mutateAsync({ groupId: Number(groupId), leadId }); setGroupId(""); toast.success("Lead added to group."); }}><FolderPlus className="mr-2 h-4 w-4" />Add</Button></div></section>}<section className="min-w-0"><Label>Current groups</Label><div className="mt-2 flex min-w-0 flex-wrap gap-2">{sharing?.groups.length ? sharing.groups.map(group => <Badge key={group.id} variant="outline" className="max-w-full whitespace-normal break-words text-left [overflow-wrap:anywhere]">{group.name}</Badge>) : <span className="text-sm text-slate-400">Not added to a group.</span>}</div></section>{sharing?.canManage && <section className="min-w-0 border-t border-slate-100 pt-5"><Label>Direct staff access</Label><div className="mt-3 space-y-2">{sharing.shares.map(item => <div key={item.id} className="flex min-w-0 items-center gap-3 rounded-xl bg-slate-50 p-2 pl-3"><div className="min-w-0 flex-1"><p className="break-words text-sm font-medium [overflow-wrap:anywhere]">{item.name || item.email}</p><p className="break-all text-xs text-slate-400">{item.email}</p></div><Button size="icon" variant="ghost" className="shrink-0" onClick={async () => { await unshare.mutateAsync({ leadId, staffId: item.userId }); toast.success("Direct access removed."); }}><Trash2 className="h-4 w-4" /></Button></div>)}</div><div className="mt-3 flex min-w-0 flex-col gap-2 sm:flex-row"><Select value={staffId} onValueChange={setStaffId}><SelectTrigger className="w-full min-w-0"><SelectValue placeholder="Select staff member" /></SelectTrigger><SelectContent>{availableStaff.map(member => <SelectItem key={member.id} value={String(member.id)}>{member.name || member.email}</SelectItem>)}</SelectContent></Select><Button className="w-full sm:w-auto" disabled={!staffId || share.isPending} onClick={async () => { await share.mutateAsync({ leadId, staffId: Number(staffId) }); setStaffId(""); toast.success("Lead shared."); }}><UserPlus className="mr-2 h-4 w-4" />Share</Button></div></section>}</div><DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>Done</Button></DialogFooter></DialogContent></Dialog>;
}
