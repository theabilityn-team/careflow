...
import { ArrowLeft, CalendarClock, FileImage, GitCompareArrows, History, Info, LockKeyhole, Mail, MapPin, MessageSquarePlus, Pencil, Phone, UserRound } from "lucide-react";
...
export default function LeadDetail() {
  ...
  return <div className="mx-auto max-w-[1380px]">
    ...
    <div className="mb-6 rounded-2xl border border-teal-100 bg-teal-50/70 p-5"><div className="flex items-start gap-3"><div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white text-teal-700 shadow-sm"><Info className="h-4 w-4" /></div><div><p className="font-semibold text-slate-900">How contact tracking works</p><div className="mt-3 grid gap-3 text-sm leading-6 text-slate-600 md:grid-cols-2"><p><strong className="text-slate-800">Most recent contact:</strong> {CONTACT_TRACKING_HELP.lastContact}</p><p><strong className="text-slate-800">Scheduled follow-up:</strong> {CONTACT_TRACKING_HELP.nextFollowUp}</p></div></div></div></div>
