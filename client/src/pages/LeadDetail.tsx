...
import { DIAGNOSIS_CATEGORY_OPTIONS, INTEREST_OPTIONS, STATE_OPTIONS, STATUS_OPTIONS, diagnosisCategoryLabel, formatDate, initials, stateLabel, statusLabel } from "@/lib/crm";
import { CONTACT_TRACKING_HELP, followUpTimingLabel, getFollowUpTiming } from "@/lib/contactTracking";
import { trpc } from "@/lib/trpc";
import { ArrowLeft, CalendarClock, FileImage, FolderPlus, GitCompareArrows, Info, LockKeyhole, Mail, MapPin, MessageSquarePlus, Pencil, Phone, Share2, Trash2, UserPlus } from "lucide-react";
...
const auditFieldLabels: Record<string, string> = {
  ...
  diagnosisCategory: "Diagnosis group", diagnosis: "Diagnosis", clinicalNotes: "Clinical notes", additionalInformation: "Additional information",
...
function buildLeadForm(lead: any) {
  return {
    ...
    country: lead.country ?? "", stateCode: lead.stateCode ?? "", diagnosisCategory: lead.diagnosisCategory ?? "", status: lead.status ?? "new", interestLevel: lead.interestLevel ?? "unknown",
  ...
  async function save() {
    ...
    try {
      const values: Record<string, unknown> = {
        ...
        country: form.country.trim() || null, stateCode: form.stateCode, diagnosisCategory: form.diagnosisCategory, interestLevel: form.interestLevel,
