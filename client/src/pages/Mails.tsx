...
export default function Mails() {
  ...
  return <div className="mx-auto max-w-[1180px]">
    ...
    <Tabs defaultValue="compose" className="gap-5">
      <TabsList className={`grid h-auto w-full ${isSuperAdmin ? "grid-cols-3" : "grid-cols-2"} rounded-xl bg-white p-1 shadow-sm sm:inline-flex sm:h-11 sm:w-fit`}><TabsTrigger value="compose" className="min-w-0 px-1 text-xs sm:px-2 sm:text-sm"><Send className="hidden sm:block" />Compose</TabsTrigger><TabsTrigger value="history" className="min-w-0 px-1 text-xs sm:px-2 sm:text-sm"><Mail className="hidden sm:block" />Sent history</TabsTrigger>{isSuperAdmin && <TabsTrigger value="template" className="min-w-0 px-1 text-xs sm:px-2 sm:text-sm"><Code2 className="hidden sm:block" />Header & footer</TabsTrigger>}</TabsList>
      ...
      <TabsContent value="compose" className="mt-0">
        <Card className="rounded-2xl border-0 bg-white shadow-[0_8px_30px_rgba(15,23,42,.045)]"><CardHeader><CardTitle>New lead email</CardTitle><p className="text-sm text-slate-500">The selected lead must be accessible to you and have an email address.</p></CardHeader><CardContent className="space-y-5">
          ...
          <div className="min-w-0 space-y-2"><Label>Lead recipient</Label><Select value={selectedLeadId} onValueChange={setSelectedLeadId}><SelectTrigger className="w-full min-w-0"><SelectValue placeholder={recipients.isLoading ? "Loading leads…" : "Select a lead"} /></SelectTrigger><SelectContent>{recipients.data?.map(lead => <SelectItem key={lead.id} value={lead.id.toString()}>{lead.firstName} {lead.lastName} · {lead.email}</SelectItem>)}</SelectContent></Select>{selectedRecipient && <p className="break-all text-xs text-slate-500">To: <strong>{selectedRecipient.email}</strong></p>}</div>
          ...
          {contentMode === "html" ? <div className="space-y-3"><div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center"><div className="min-w-0"><Label>HTML message</Label><p className="mt-1 break-words text-xs text-slate-400">This source is sanitized again when the email is sent.</p></div><div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto"><Button type="button" variant={!htmlPreview ? "default" : "outline"} size="sm" onClick={() => setHtmlPreview(false)}><Code2 className="mr-2 h-3.5 w-3.5" />Edit HTML</Button><Button type="button" variant={htmlPreview ? "default" : "outline"} size="sm" onClick={() => setHtmlPreview(true)}><Eye className="mr-2 h-3.5 w-3.5" />Preview</Button></div></div>{htmlPreview ? <iframe title="Email draft preview" sandbox="" srcDoc={bodyHtml} className="h-[min(460px,45dvh)] w-full rounded-xl border border-slate-200 bg-white sm:h-[460px]" /> : <Textarea className="font-mono text-xs" value={bodyHtml} onChange={event => setBodyHtml(event.target.value)} rows={20} maxLength={250_000} />}</div> : <div className="space-y-2"><Label>Message</Label><Textarea value={bodyText} onChange={event => setBodyText(event.target.value)} rows={11} maxLength={50_000} placeholder="Write the email message" /></div>}
          ...
          <div className="flex justify-end"><Button onClick={sendEmail} disabled={send.isPending || !selectedLeadId || !subject.trim() || (contentMode === "html" ? !bodyHtml.trim() : !bodyText.trim())} className="w-full bg-teal-700 hover:bg-teal-800 sm:w-auto">{send.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Send className="mr-2 h-4 w-4" />}Send email</Button></div>
        </CardContent></Card>
      </TabsContent>

      <TabsContent value="history" className="mt-0">
        <Card className="rounded-2xl border-0 bg-white shadow-sm"><CardHeader><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><CardTitle>Sent email history</CardTitle><p className="mt-1 text-sm text-slate-500">{access?.role === "super_admin" ? "All outbound emails across CareFlow." : "Emails sent from your assigned account."}</p></div><div className="flex flex-wrap gap-2"><Badge className="bg-sky-50 text-sky-700 hover:bg-sky-50">{openedMessages} opened</Badge><Badge className="bg-violet-50 text-violet-700 hover:bg-violet-50">{clickedMessages} clicked</Badge></div></div></CardHeader><CardContent><EmailEngagementNotice className="mb-4" />{history.isLoading ? <PageLoading /> : !history.data?.length ? <EmptyState title="No emails sent yet" description="Successful and failed send attempts will appear here." /> : <div className="divide-y divide-slate-100">{history.data.map(item => <button key={item.id} onClick={() => setSelectedMessageId(item.id)} className="flex min-h-14 w-full min-w-0 flex-col gap-3 py-4 text-left transition-opacity hover:opacity-70 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><div className="flex min-w-0 flex-wrap items-center gap-2"><p className="min-w-0 break-words font-semibold text-slate-900 [overflow-wrap:anywhere]">{item.subject}</p>{item.productName && <Badge variant="outline" className="max-w-full whitespace-normal border-cyan-200 bg-cyan-50 text-left text-cyan-800">{item.productName}</Badge>}{item.templateName && <Badge variant="outline" className="max-w-full whitespace-normal text-left">{item.templateName}</Badge>}</div><p className="mt-1 break-words text-sm text-slate-500 [overflow-wrap:anywhere]">To {item.recipientName} · <span className="break-all">{item.recipientEmail}</span></p><p className="mt-1 break-words text-xs text-slate-400 [overflow-wrap:anywhere]">From {item.senderName} · <span className="break-all">{item.fromEmail}</span></p><div className="mt-2"><EmailEngagementBadges email={item} /></div></div><div className="flex flex-wrap items-center gap-3"><Badge variant="outline" className={item.status === "sent" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-rose-200 bg-rose-50 text-rose-700"}>{item.status === "sent" ? <CheckCircle2 className="mr-1 h-3 w-3" /> : <AlertTriangle className="mr-1 h-3 w-3" />}{item.status}</Badge><span className="text-xs text-slate-400">{formatEasternDate(item.sentAt, true)}</span></div></button>)}</div>}</CardContent></Card>
      </TabsContent>

      {isSuperAdmin && <TabsContent value="template" className="mt-0"><GlobalEmailFrameEditor /></TabsContent>}
    </Tabs>

    <Dialog open={Boolean(selectedMessageId)} onOpenChange={open => { if (!open) setSelectedMessageId(undefined); }}><DialogContent className="flex max-h-[calc(100dvh-1rem)] max-w-3xl flex-col overflow-hidden"><DialogHeader className="shrink-0"><DialogTitle>{message.data?.subject || "Email details"}</DialogTitle><DialogDescription>{message.data ? <>To {message.data.recipientName} · <span className="break-all">{message.data.recipientEmail}</span> · {formatEasternDate(message.data.sentAt, true)}</> : "Loading email…"}</DialogDescription></DialogHeader>{message.data && <div className="min-h-0 space-y-4 overflow-y-auto overscroll-contain"><div className="flex min-w-0 flex-wrap gap-2 text-xs"><Badge variant="outline" className="max-w-full whitespace-normal break-all text-left">From {message.data.fromEmail}</Badge>{message.data.productName && <Badge variant="outline" className="max-w-full whitespace-normal border-cyan-200 bg-cyan-50 text-left text-cyan-800">Product: {message.data.productName}</Badge>}{message.data.templateName && <Badge variant="outline" className="max-w-full whitespace-normal text-left">Template: {message.data.templateName}</Badge>}<Badge variant="outline">{message.data.status}</Badge></div>{message.data.status === "sent" && <EmailEngagementDetails email={message.data} />}{message.data.error && <Alert variant="destructive"><AlertTriangle className="h-4 w-4" /><AlertTitle>Delivery failed</AlertTitle><AlertDescription className="break-words [overflow-wrap:anywhere]">{message.data.error}</AlertDescription></Alert>}<iframe title="Sent email preview" sandbox="" srcDoc={message.data.bodyHtml} className="h-[min(420px,45dvh)] w-full rounded-xl border border-slate-200 bg-white sm:h-[420px]" /></div>}</DialogContent></Dialog>
  </div>;
}
