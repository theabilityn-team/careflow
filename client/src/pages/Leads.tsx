    ...
    <Card className="rounded-2xl border-0 bg-white shadow-[0_8px_30px_rgba(15,23,42,.045)]">
      <CardContent className="p-0">
        <div className="flex flex-col gap-3 border-b border-slate-100 p-5 sm:flex-row">
          <div className="relative flex-1"><Search className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><Input value={search} onChange={e => setSearch(e.target.value)} className="h-11 border-slate-200 pl-10" placeholder="Search by name, email, or phone…" /></div>
        ...
        {isLoading ? <div className="p-6"><PageLoading /></div> : error ? <div className="m-6 rounded-xl bg-rose-50 p-4 text-sm text-rose-700">{error.message}</div> : !data?.length ? <div className="p-6"><EmptyState title="No matching leads" description="Try another filter or scan documents to create a new reviewed record." action={access?.permissions.scanDocuments && access.permissions.viewClinical ? <Button onClick={() => navigate("/scan")} variant="outline">Scan documents</Button> : undefined} /></div> : <div className="overflow-x-auto"><Table><TableHeader><TableRow className="border-slate-100 hover:bg-transparent"><TableHead className="pl-6">Lead</TableHead><TableHead>Status</TableHead><TableHead>Interest</TableHead><TableHead>Contact</TableHead><TableHead>Next follow-up</TableHead><TableHead className="w-12" /></TableRow></TableHeader><TableBody>{data.map(lead => <TableRow key={lead.id} onClick={() => navigate(`/leads/${lead.id}`)} className="cursor-pointer border-slate-100"><TableCell className="py-4 pl-6"><div className="flex items-center gap-3"><Avatar className="h-10 w-10"><AvatarFallback className="bg-teal-50 text-xs font-semibold text-teal-800">{initials(lead.firstName, lead.lastName)}</AvatarFallback></Avatar><div><p className="font-semibold text-slate-900">{lead.firstName} {lead.lastName}</p><p className="text-xs text-slate-400">Added {formatDate(lead.createdAt)}</p></div></div></TableCell><TableCell><StatusPill value={lead.status} /></TableCell><TableCell><span className={`inline-flex items-center gap-1.5 text-sm font-medium ${lead.interestLevel === "hot" ? "text-amber-700" : "text-slate-600"}`}>{lead.interestLevel === "hot" && <Flame className="h-3.5 w-3.5" />}{INTEREST_OPTIONS.find(([v]) => v === lead.interestLevel)?.[1]}</span></TableCell><TableCell><p className="max-w-56 truncate text-sm text-slate-700">{lead.email || lead.phone || "—"}</p></TableCell><TableCell><span className="text-sm text-slate-600">{formatDate(lead.nextFollowUpAt, true)}</span></TableCell><TableCell><ChevronRight className="h-4 w-4 text-slate-300" /></TableCell></TableRow>)}</TableBody></Table></div>}
      </CardContent>
    </Card>
  </div>;
}
