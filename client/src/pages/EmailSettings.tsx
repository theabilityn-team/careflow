...
export default function EmailSettings() {
  ...
  return <div className="mx-auto max-w-[1050px]">
    ...
    <Card className="mb-6 rounded-2xl border-0 bg-white shadow-sm"><CardContent className="p-5"><Label>Account to configure</Label><Select value={selectedUserId?.toString()} onValueChange={value => setSelectedUserId(Number(value))}><SelectTrigger className="mt-2"><SelectValue placeholder="Select an account" /></SelectTrigger><SelectContent>{accounts.data?.map(account => <SelectItem key={account.userId} value={account.userId.toString()}>{account.name} · {account.accountEmail || "No account email"}{!account.isActive ? " · Inactive" : ""}</SelectItem>)}</SelectContent></Select></CardContent></Card>
    ...
    <div className="mb-6 grid gap-4 md:grid-cols-3">
      ...
      <Card className="rounded-2xl border-0 bg-white shadow-sm"><CardContent className="p-5">{verified ? <CheckCircle2 className="h-5 w-5 text-emerald-600" /> : <AlertTriangle className="h-5 w-5 text-amber-600" />}<p className="mt-4 text-sm font-semibold">{verified ? "Connection verified" : "Connection not verified"}</p><p className="mt-2 text-xs leading-5 text-slate-500">{managed?.lastTestedAt ? `Last tested ${new Date(managed.lastTestedAt).toLocaleString()}` : "Save the mailbox, then test the connection."}</p></CardContent></Card>
    </div>
