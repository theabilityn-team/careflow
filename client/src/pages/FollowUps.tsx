...
function FollowUpRow({ item, overdue, canManage, onOpen, onComplete, onEdit, onDelete }: {
  ...
  return <div className="flex min-w-0 flex-col gap-4 px-4 py-5 sm:flex-row sm:items-center sm:px-6">
    <button onClick={onOpen} className="flex min-h-11 min-w-0 flex-1 items-start gap-3 text-left transition-opacity hover:opacity-75 sm:items-center sm:gap-4">
      <Avatar className="h-10 w-10 shrink-0"><AvatarFallback className="bg-teal-50 text-xs font-semibold text-teal-800">{initials(item.firstName, item.lastName)}</AvatarFallback></Avatar>
      ...
      <div className="hidden shrink-0 sm:block"><StatusPill value={item.status} /></div><ArrowRight className="mt-3 h-4 w-4 shrink-0 text-slate-300 sm:mt-0" />
    </button>
...
export default function FollowUps() {
  ...
  return <div className="mx-auto max-w-[1280px]">
    ...
    {automationStatus && (!automationStatus.emailConfigured || !automationStatus.scheduleConfigured) && <div className="mb-6 flex min-w-0 flex-col gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950 sm:flex-row sm:items-start"><AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-amber-700" /><div className="min-w-0 flex-1"><p className="font-semibold">Email reminder automation is not fully activated</p><p className="mt-1 break-words leading-6 [overflow-wrap:anywhere]">{automationStatus.scope === "staff" ? "In-app reminders are working. Super Admin must activate your assigned SMTP mailbox; you can review and test it under Email settings." : `Super Admin SMTP is ${automationStatus.adminEmailConfigured ? "verified" : "not verified"}. ${automationStatus.configuredStaff} of ${automationStatus.totalStaff} active staff SMTP accounts are verified. Super Admin manages every sender.`}{!automationStatus.scheduleConfigured ? " The scheduled reminder processor must also be activated after deployment." : ""}</p></div><Button size="sm" variant="outline" className="w-full bg-white sm:w-auto" onClick={() => navigate("/email-settings")}>{automationStatus.scope === "staff" ? "View Email status" : "Manage SMTP"}</Button></div>}
    {dueNotifications.length > 0 && <Card className="mb-6 min-w-0 overflow-hidden rounded-2xl border-0 bg-slate-950 text-white shadow-[0_12px_40px_rgba(15,23,42,.15)]"><CardContent className="p-0"><div className="flex min-w-0 flex-col items-start justify-between gap-3 border-b border-slate-800 px-4 py-5 min-[360px]:flex-row min-[360px]:items-center sm:px-6"><div className="flex min-w-0 items-center gap-3"><BellRing className="h-5 w-5 shrink-0 text-amber-300" /><div className="min-w-0"><h2 className="font-semibold">Two-hour notifications</h2><p className="mt-1 break-words text-xs text-slate-400">Appointments requiring your attention now</p></div></div><Badge className="bg-amber-300 text-slate-950 hover:bg-amber-300">{dueNotifications.length} unread</Badge></div><div className="divide-y divide-slate-800">{dueNotifications.map(item => <div key={item.id} className="flex min-w-0 flex-col gap-4 px-4 py-5 sm:flex-row sm:items-center sm:px-6"><button onClick={() => navigate(`/leads/${item.leadId}`)} className="min-h-11 min-w-0 flex-1 text-left"><p className="break-words font-semibold [overflow-wrap:anywhere]">{item.firstName} {item.lastName}</p><p className="mt-1 break-words text-sm text-slate-400 [overflow-wrap:anywhere]">Scheduled {formatDate(item.scheduledFor, true)} · {stateLabel(item.stateCode)} · {diagnosisCategoryLabel(item.diagnosisCategory)}</p><p className="mt-2 flex min-w-0 flex-wrap items-start gap-2 break-words text-xs text-slate-500"><Mail className="mt-0.5 h-3.5 w-3.5 shrink-0" /><span>Staff email: {item.staffEmailStatus} · Lead email: {item.leadEmailStatus}</span></p></button><Button size="sm" variant="outline" className="w-full border-slate-700 bg-slate-900 text-white hover:bg-slate-800 hover:text-white sm:w-auto" onClick={async () => { try { await markRead.mutateAsync({ id: item.id }); toast.success("Notification marked as read."); } catch (markError) { toast.error(markError instanceof Error ? markError.message : "Unable to mark notification."); } }}><Check className="mr-2 h-4 w-4" />Mark read</Button></div>)}</div></CardContent></Card>}
    ...
    <Tabs value={tab} onValueChange={setTab} className="gap-5">
      ...
      <TabsContent value="queue" className="mt-0">{error ? <div className="break-words rounded-xl bg-rose-50 p-4 text-sm text-rose-700 [overflow-wrap:anywhere]">{error.message}</div> : <div className="grid gap-6 lg:grid-cols-2"><Group title="Overdue" items={overdue as FollowUpItem[]} isOverdue /><Group title="Upcoming" items={upcoming as FollowUpItem[]} /></div>}</TabsContent>
      <TabsContent value="calendar" className="mt-0"><div className="grid gap-6 lg:grid-cols-[430px_1fr]">
        <Card className="rounded-2xl border-0 bg-white shadow-sm"><CardContent className="p-4 sm:p-5"><div className="mb-4"><p className="font-semibold text-slate-950">Follow-up calendar</p><p className="mt-1 text-sm text-slate-500">Dates with reminders are highlighted in Eastern Time.</p></div><Calendar timeZone={CAREFLOW_TIME_ZONE} mode="single" month={calendarMonth} onMonthChange={month => { setCalendarMonth(month); setSelectedDate(month); }} selected={selectedDate} onSelect={setSelectedDate} modifiers={{ hasFollowUps: eventDates }} modifiersClassNames={{ hasFollowUps: "font-bold ring-2 ring-teal-500/30 bg-teal-50" }} className="mx-auto w-full [--cell-size:--spacing(8)] sm:[--cell-size:--spacing(10)]" /></CardContent></Card>
