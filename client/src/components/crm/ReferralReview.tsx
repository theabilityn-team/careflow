...
export function ReferralReview({ value, onChange, documentCategory, onDocumentCategoryChange }: {
  ...
  return <section className="rounded-2xl border border-indigo-100 bg-indigo-50/45 p-5 sm:p-6">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[.16em] text-indigo-700">Referral document review</p><h3 className="mt-2 text-lg font-semibold text-slate-950">Verify referral, insurance, providers, and codes</h3><p className="mt-1 text-sm leading-6 text-slate-600">Patient fields remain separate from provider and facility information. Empty fields are not saved.</p></div><div className="w-full space-y-2 sm:w-52"><Label>Detected document type</Label><Select value={documentCategory} onValueChange={onDocumentCategoryChange}><SelectTrigger className="bg-white"><SelectValue /></SelectTrigger><SelectContent>{DOCUMENT_TYPE_OPTIONS.map(([key, label]) => <SelectItem key={key} value={key}>{label}</SelectItem>)}</SelectContent></Select></div></div>
