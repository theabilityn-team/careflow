...
export function CopyContactButton({ value, label, compact = false }: { value: string; label: "email" | "phone"; compact?: boolean }) {
  ...
  return <Button
    ...
    title={`Copy ${label}`}
    className={compact ? "h-8 w-8 shrink-0 p-0 text-slate-400 hover:bg-teal-50 hover:text-teal-700" : "h-8 shrink-0 gap-1.5 px-2.5 text-xs text-slate-500 hover:bg-teal-50 hover:text-teal-700"}
  >
