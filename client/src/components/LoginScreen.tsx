...
export default function LoginScreen() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      await login.mutateAsync(superAdminMode
        ? { mode: "super_admin", identifier: SUPER_ADMIN_EMAIL, password }
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#f4f7f6] text-slate-950">
      ...
      <div className="relative mx-auto grid min-h-screen max-w-6xl items-center gap-10 px-4 py-8 sm:px-6 sm:py-12 lg:grid-cols-[1.15fr_.85fr] lg:gap-12">
        <div className="min-w-0">
          ...
          <h1 className="max-w-2xl break-words text-4xl font-semibold leading-[1.06] tracking-[-.045em] sm:text-6xl">Turn documents into trusted customer records.</h1>
        ...
        <div className="min-w-0 rounded-[2rem] bg-slate-950 p-5 text-white shadow-2xl shadow-slate-900/15 sm:p-8">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-300">{superAdminMode ? "Enter the Super Admin email and private password." : "Use the email address and password from your staff invitation."}</p>
          <form onSubmit={submit} className="mt-6 space-y-4">
            ...
            <Button type="submit" disabled={login.isPending || (!superAdminMode && !email)} size="lg" className="h-auto min-h-11 w-full whitespace-normal bg-teal-500 py-2.5 text-center leading-5 text-slate-950 hover:bg-teal-400">{login.isPending ? <Loader2 className="mr-2 h-4 w-4 shrink-0 animate-spin" /> : <LockKeyhole className="mr-2 h-4 w-4 shrink-0" />}{superAdminMode ? "Sign in as Super Admin" : "Sign in securely"}</Button>
          </form>
