...
export default function ForgotPasswordDialog({ initialEmail }: { initialEmail?: string }) {
  ...
  return <Dialog open={open} onOpenChange={value => { setOpen(value); if (value) setEmail(initialEmail ?? ""); else setSubmitted(false); }}>
    <DialogTrigger asChild><button type="button" className="inline-flex min-h-11 items-center rounded-lg px-2 text-sm font-medium text-teal-300 transition-colors hover:bg-white/5 hover:text-teal-200 md:min-h-8"><HelpCircle className="mr-1.5 h-4 w-4" />Forgot password?</button></DialogTrigger>
